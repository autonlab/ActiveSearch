...
class LapSVMOptions:
	# Options class with modifyable attributes (can add attributes)
	def __init__ (self):
		self.Verbose =True

class LapSVMData:

	def __init__(self, X, Y, K = None, L = None, Kv=None, Yv=None, sparse=True):
		self.X = X
		self.Y = Y

		if K is None:
			self.K = ss.csr_matrix(X.dot(X.T))
		if L is None:
			if sparse:
...
def LapSVMP(data, options=LapSVMOptions()):
	...
	b = options.InitBias
	if options.Cg == 0:
		alpha,b,t,sec,lsiters,_ = newton (data,options,alpha,b,oc)
		stats = []
	elif options.Cg == 1:
...
# {pcg} trains the classifier using preconditioned conjugate gradient.
	...
	lsiters = np.zeros((options.MaxIter,1))
	valerr_prev = 1
	obj_prev = obj0
	yfx_unlabeled_prev = np.ones(u,1).astype(bool)
	...
	while True:
		...
		if (t-1)%options.CgStopIter == options.CgStopIter-1:
			if options.CgStopType == 1: # stability
				...
				yfx_unlabeled = np.sign(Kalpha[unlabeled] + b)
				diff_unlabeled = 1 - np.sum(yfx_unlabeled == yfx_unlabeled_prev) / u
				if (valerr > (valerr_prev - options.CgStopParam[0])) and 
					diff_unlabeled < options.CgStopParam[1]:
						break
				valerr_prev = valerr.copy()
				yfx_unlabeled_prev = yfx_unlabeled.copy()
			elif options.CgStopType == 4: # gradient norm
		...
		be = max(0,(Kgn.T.dot(gn-go) + gn_b.dot(gn_b-go_b))/(Kgo.T.dot(go) + go_b**2))
		d = be*d + gn
		d_b = be*d_b + gn_b
		Kstep = be * Kstep + Kgn
		go = gn.copy()
...
def linesearch(data,labeled=None,step=None,step_b=None,Kalpha=None,b=None,gamma_A=None,gamma_I=None,Kstep=None,LKalpha=None,hinge=None,oc=None):
	...
	if hinge and len(i) > 0:
		...
		while not_got_it:
			...
			# updating intercepts
			left = left + j*outstep[deltas_map[i]].dot(out_minus_Y[deltas_map[i]])
			right = right + j*outstep[deltas_map[i]].dot(outstep[deltas_map[i]])
