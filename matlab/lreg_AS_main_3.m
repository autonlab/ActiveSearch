function [hits,selected] = lreg_AS_main_3(X,deg,dim,alpha,labels,options)
...
pai = 0.05; %prior target probability
...
if(isfield(options,'start_point'))
	...
	if(labels(start_point) ~= 1)
		fprintf(2,'Warning: user-specified start point %d is not positive. Change it to positive. \n', start_point);
