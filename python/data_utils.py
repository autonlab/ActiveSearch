...
def whiten_data(X, sparse=True, rtn_W=False, thresh=1e-6):
  if sparse:
    X = np.array(X.todense())

  Xm = X - X.mean(1).reshape(-1, 1)
  A = Xm.dot(Xm.T) / X.shape[1]
  d, V = nlg.eigh(A)
  non_zero = np.where(d > thresh)[0]

  D = np.diag(1. / np.sqrt(d[non_zero]))
  W = D.dot(V[:, non_zero].T)
  Xw = W.dot(Xm)
  if sparse:
    Xw = ss.csr_matrix(Xw)
...
def whiten_data2(X, sparse=True, thresh=1e-6):
  # Incorrect.
  ...
  r = X.shape[0]
  Xm = X - X.mean(1).reshape(-1, 1)
  U, s, Vt = np.linalg.svd(Xm.T, full_matrices=0)
  num_zero = (s < thresh).sum()
  Xw = (U[:, :r-num_zero].dot(Vt[:r-num_zero,:])).T
