...
results_dir = osp.join(du.results_dir, 'aaai17/pcmac/expts')


def test_pcmac (arg_dict):
  ...
  t1 = time.time()
  cl = 0
  X0,labels,categories = du.load_pcmac()
  # X_norms = np.sqrt((X0*X0).sum(axis=0)).squeeze()
  # X0 = (X0/X_norms) # Normalization
  
  #lcl1 = (labels == cl1)
  #lcl2 = (labels == cl2)
  #X0 = np.c_[X0[:, lcl1], X0[:, lcl2]]
  #Y0 = np.array([1] * lcl1.sum() + [0] * lcl2.sum())
  Y0 = (labels == cl) 
  # X0,W = du.whiten_data(X0, sparse=sparse, rtn_W=True, thresh=1e-6)
  print('Time taken to load segmentation data: %.2f'%(time.time()-t1))

  t1 = time.time()
  rfc = GRF.GaussianRandomFeatures(X0.shape[0], X0.shape[0]*10, gammak=0.5)
...
def test_pcmac_imf (arg_dict):
  ...
  t1 = time.time()
  cl = 0
  X0,Y0,classes = du.load_segmentation()
  Y0 = (Y0 == cl) 
  # import IPython
  # IPython.embed()
  ...
  t1 = time.time()
  rfc = GRF.GaussianRandomFeatures(X0.shape[0], X0.shape[0]*10, gammak=1.0)
  RX0 = rfc.computeRandomFeatures(X0.T).T
  print('Time taken to convert to fourier features: %.2f'%(time.time() - t1))
...
if __name__ == '__main__':
  ...
  if len(sys.argv) > 3:
    try:
      ...
      prev = 0.05
    if prev < 0 or prev > 0.2:
      prev = 0.2

  test_funcs = {1:test_pcmac}#, 2:test_seg_imf}
