...
hubs = 3; followers = 10;
f0 = round(followers/2);
f1 = followers - f0;

X = zeros(hubs*followers, (hubs-1)*followers + hubs);
Yfull = [];
t = 0;
for i=1:(hubs-1)
	idx = (i-1)*(followers+1) + 1;
	X((i-1)*followers+1:i*followers, idx) = 1;
	for j = 1:followers
		X((i-1)*followers+j, idx+j) = 1;
		X(i*followers+j, idx+j) = 1;
	end
	if t == 0
		Yfull = [Yfull, zeros(1,f0+1), ones(1,f1)];
		t = 1;
	else
		Yfull = [Yfull, ones(1,f1+1), zeros(1,f0)];
		t = 0;
	end
end

X((hubs-1)*followers+1:hubs*followers, (hubs-1)*followers + hubs) = 1;
if t == 0
	Yfull = [Yfull, 0];
else
	Yfull = [Yfull, 0];
...
options.start_point = 11;
options.n_conncomp = b;
options.num_evaluations = 15;
